...
import AdmissionFormUserOriginal, { IAdmissionFormUserOriginal} from './models/AdmissionFormUserOriginal'; // Adjust the import path as needed
...
const updateAdmissionFormById = async (id: string, updateData: Partial<IAdmissionFormUserOriginal>) => {
  try {
    const updatedDocument = await AdmissionFormUserOriginal.findByIdAndUpdate(
      id,
      updateData,
      {
        new: true,
...
export default async function handler(req: NextApiRequest, res: NextApiResponse) {
  ...
  if (req.method === 'PUT') {
    try {
      ...
      const document = await AdmissionFormUserOriginal.findById(id);

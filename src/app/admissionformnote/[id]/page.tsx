...
import AdmissionFormNote from "../../../components/forms/AdmissionFormNote";
...
async function getFormById(id: string): Promise<FormData> {
  ...
  try {
    const response = await fetch(`https://school-iota-three.vercel.app/api/admissionformnote/${id}?t=${timestamp}`, { method: 'GET' });
    if (!response.ok) {

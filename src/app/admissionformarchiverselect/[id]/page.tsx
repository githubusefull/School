...
async function getFormById(id: string): Promise<FormData> {
  ...
  try {
    const response = await fetch(`https://school-iota-three.vercel.app/api/admissionformarchiverselect/${id}?t=${timestamp}`, { method: 'GET' });
    if (!response.ok) {
...
export default async function FormID({ params }: FormIDProps) {
  ...
  return (
    ...
    <Renouveler formAfSelect={formAfSelect}/>
